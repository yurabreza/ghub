...
val dataModule = module {
    ...
    single { getRepositoriesRepository(get(), get(), get()) }
}

fun getRepositoriesDataBase(applicationContext: Context): RepositoriesDatabase = Room.databaseBuilder(
    applicationContext,
    RepositoriesDatabase::class.java, DATABASE_NAME
).build()
...
fun getRepositoriesRepository(
    ...
    prefs: IPreferencesStorage
): IRepositoriesRepository {
    return RepositoriesRepository(repositoryApi, repositoriesDatabase, prefs)
}
